...
class Engine(object):
    def __init__(self, state={}):
        ...
        if self._state('workers') is None:
            self.state['workers'] = 4  # giảm từ 25 → 4 (an toàn hơn)
        ...
        # meters — dùng tự viết thay vì torchnet để tránh dependency
    ...
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        # FIX: loại bỏ torch.autograd.Variable (deprecated) và volatile
    ...
    def train(self, data_loader, model, criterion, optimizer, epoch):
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            if self.state['use_gpu']:
                # FIX: async keyword deprecated → non_blocking
    ...
    def validate(self, data_loader, model, criterion):
        ...
        with torch.no_grad():  # FIX: thay volatile bằng no_grad context
            for i, (input, target) in enumerate(data_loader):
                self.state['iteration'] = i
                self.state['data_time_batch'] = time.time() - end
                self.state['data_time'].add(self.state['data_time_batch'])
...
class GCNMultiLabelMAPEngine(MultiLabelMAPEngine):
    """Engine dành riêng cho GCNResnet — override on_forward và on_start_batch."""

    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        # FIX: bỏ Variable/volatile, dùng .float() trực tiếp
        ...
        if training:
            optimizer.zero_grad()
            self.state['loss'].backward()
            # FIX: clip_grad_norm → clip_grad_norm_ (underscore, in-place)
    ...
    def on_start_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        input = self.state['input']
        self.state['feature'] = input[0]   # ảnh
        self.state['out'] = input[1]        # filename / path
        self.state['input'] = input[2]      # word_vec (inp)


# ── Helper class thay torchnet.AverageValueMeter ─────────────────────────
class _AverageMeter:
    def __init__(self):
        self.reset()
    ...
    def value(self):
        # tương thích với tnt.meter.AverageValueMeter().value() → trả tuple (mean, std)
        return (self.avg, 0.0)
